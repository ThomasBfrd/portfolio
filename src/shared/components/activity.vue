...
<template>
    <section class="border-b border-zinc-800">
        <ul class="flex flex-col gap-8">
            <li v-for="experience in activities" :key="experience.company" class="flex flex-col gap-4 [&:not(:last-child)]:border-b border-zinc-800 pb-6">
            ...
            <div class="flex flex-wrap gap-2 mt-2">
                ...
                class="w-[140px] text-center text-[14px] text-zinc-300 border border-zinc-600 rounded-full bg-zinc-800 cursor-pointer hover:bg-zinc-700 hover:text-white transition-colors duration-200 p-1">
                {{ techno }}
                </span>
            </div>
