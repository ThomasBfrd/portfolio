           ...
           <li v-for="experience in activities" :key="experience.company"
              ...
              <div class="flex flex-wrap gap-2 mt-2">
                    <span v-for="techno in experience.tools" :key="techno"
                       class="w-[125px] text-center text-[14px] border border-border rounded-full bg-background cursor-pointer hover:bg-border hover:text-foreground transition-colors duration-200 p-1">
                        {{ techno }}
                    </span>
                </div>
